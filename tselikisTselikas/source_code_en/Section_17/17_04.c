...
void Build_Pkt(int IP_src[], int IP_dst[], int port)
{
	...
	pkt[21] = 1500 & 0xFF;
	pkt[22] = port >> 8; /* TCP Destination Port. */
	pkt[23] = port & 0xFF; 
	pkt[33] = 2; /* SYN bit. */
	pkt[34] = 0xFF; /* To get the max value of the Window field set its bits to 1. */
	pkt[35] = 0xFF;
	/* The values of the CheckSum and Urgent Pointer fields are set in positions 36-40, therefore the total length of the IP packet is 40 bytes. */	
